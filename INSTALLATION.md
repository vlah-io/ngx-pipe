...
```
  ng new ngx-pipe --createApplication=false --prefix=vlahio
  cd ngx-pipe
```
...
```
  cd ngx-pipe
  ng generate library ngx-pipe --prefix=vlahio
```
...
```
  cd ngx-pipe
  ng generate application ngx-pipe-test
```
...
4. When using directives depended on `CSS` in the `test` application don't forget to add the necessary imports (see the example bellow).
   ```
    @import "../../ngx-pipe/src/assets/css/ngx-pipe.css";
   ```
